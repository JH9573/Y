...
# 对话状态
NAME, HOST, PORT, USERNAME, AUTH_TYPE, CREDENTIAL, JUMP_CHOICE = range(7)
...
async def step_credential(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # 已登记过跳板机时让用户选用;否则直接直连
    async with crud.session() as s:
        jumps = await crud.list_jump_hosts(s)
    if not jumps:
        await context.bot.send_message(
            chat_id=update.effective_chat.id, text="开始测试连通性…"
        )
        return await _finalize(update, context)

    rows = [
        [InlineKeyboardButton(
            f"🪜 {j.name} ({j.username}@{j.host})",
            callback_data=f"jumpsel:{j.id}",
        )]
        for j in jumps
    ]
    rows.append([InlineKeyboardButton("直连(不使用跳板机)", callback_data="jumpsel:0")])
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text="选择连接方式(跳板机可在 服务器管理 → 跳板机管理 中登记):",
...
async def step_jump_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if jump_id == 0:
        context.user_data[KEY]["jump_host_id"] = None
        await query.edit_message_text("直连,不使用跳板机。开始测试连通性…")
        return await _finalize(update, context)
    async with crud.session() as s:
        jump = await crud.get_jump_host(s, jump_id)
    if jump is None:
        await query.edit_message_text("所选跳板机不存在(可能刚被删除),改为直连。开始测试连通性…")
        context.user_data[KEY]["jump_host_id"] = None
        return await _finalize(update, context)
    context.user_data[KEY]["jump_host_id"] = jump.id
    await query.edit_message_text(f"使用跳板机「{jump.name}」。开始测试连通性…")
    return await _finalize(update, context)
...
async def _finalize(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if jump_host_id is not None:
        async with crud.session() as s:
            jump = await crud.get_jump_host(s, jump_host_id)
        if jump is None:
            jump_host_id = None
    ...
    if not ok:
        ...
        if jump is not None:
            hint = f"请检查服务器信息以及跳板机「{jump.name}」是否可达后重试。"
        await context.bot.send_message(
            chat_id=chat_id,
...
def register(application, ctx) -> None:
    conv = ConversationHandler(
        ...
        states={
            ...
            JUMP_CHOICE: [CallbackQueryHandler(step_jump_choice, pattern=r"^jumpsel:\d+$")],
        },
        fallbacks=[
